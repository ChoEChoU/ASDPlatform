...
checkpoint_dir = f"/home/lmtna99/ASDPlatform/mmaction2/scratch/output/second"  # Specify your checkpoint directory
...
#     modified_pretrained_weights = {}
    
#     for key, value in pretrained_weights['state_dict'].items():
#         new_key = key
#         new_key = key.replace('module.', 'backbone.')
#         new_key = re.sub(r"\.bn(\d+)\.", r".conv\1.bn.", new_key)
#         new_key = re.sub(r"downsample\.(\d+)\.bn", r"downsample.\1.conv.bn", new_key)
#         if 'conv' in new_key and not re.search(r'conv\d+\.bn', new_key):
#             new_key = re.sub(r'(conv\d+)(\.weight|\..*)', r'\1.conv\2', new_key)
    
#     modified_pretrained_weights[new_key] = value
    
#     model.load_state_dict(modified_pretrained_weights, strict=False)
    
#     return model

for fold in range(num_folds):
    print(f"Running for fold {fold + 1}")

    # Load Datasets
    train_dataset = MyDataset(f'/home/lmtna99/Data/crop_data/IF2004/fusion/ASD_HR_fold_{fold+1}_train.txt', transform=spatial_transform, temporal_transform=temporal_transform, is_train=True)
    val_dataset = MyDataset(f'/home/lmtna99/Data/crop_data/IF2004/fusion/ASD_HR_fold_{fold+1}_validation.txt', transform=spatial_transform, temporal_transform=temporal_transform, is_train=False)
    test_dataset = MyDataset(f'/home/lmtna99/Data/crop_data/IF2004/fusion/ASD_HR_fold_{fold+1}_test.txt', transform=spatial_transform, temporal_transform=temporal_transform, is_train=False)
    ...
    freeze_except_specific_layers(i3d_model, ['backbone.layer4', 'cls_head'])
    ...
    optimizer = optim.SGD(i3d_model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=0.01)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            check_epoch = epoch + 1
            checkpoint_path = os.path.join(checkpoint_dir, f"fold{fold+1}/checkpoint_epoch_{check_epoch}.pth")
        ...
        if val_accuracy > best_accuracy:
            ...
            best_epoch = epoch
            best_model_path = os.path.join(checkpoint_dir, f"fold{fold+1}/best_model.pth")
    ...
    last_model_path = os.path.join(checkpoint_dir, f"fold{fold+1}/last_model.pth")
    ...
    cm_save_path = os.path.join(checkpoint_dir, f'fold{fold+1}/last_confusion_matrix.png')

    # Confusion Matrix 그리기 및 저장
    plot_confusion_matrix(cm, class_names, cm_save_path)

    auroc_plot_path = os.path.join(checkpoint_dir, f'fold{fold+1}/last_auroc_curve.png')
